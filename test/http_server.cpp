...
std::string ReadFile(const std::string &path) {
  ...
  // 【新增】检查 tellg 是否成功
  if (pos == -1) {
    LOG_ERROR << "tellg failed for file: " << path;
    return "";
  }

  std::streamsize flength = static_cast<std::streamsize>(pos);
...
std::string BuildFileHtml() {
  ...
  for (auto filename : filelist) {
    ...
    file += "<tr><td>" + filename + "</td>" + "<td>" + "<a href=\"/download/" +
            filename + "\">下载</a>" + "<a href=\"/delete/" + filename +
            "\">删除</a>" + "</td></tr>" + "\n";
  }
...
void HttpResponseCallback(const HttpRequest &request, HttpResponse *response) {
  ...
  if (request.method() == HttpRequest::Method::kGet) {
    ...
    if (url == "/login") {
      ...
      std::string username =
          rqbody.substr(usernamePos, usernameEndPos - usernamePos);
      std::string password =
          rqbody.substr(passwordPos, passwordEndPos - passwordPos);
